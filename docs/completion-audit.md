...
| Production pipeline avoids unrecoverable panic paths | Source hygiene scans production portions of core model/pipeline/readout/runtime/visualization/wasm and Bevy wrapper sources and rejects `panic!`, `.unwrap()`, and `.expect(` outside test modules; greedy selection readback returns a clean stop instead of panicking on malformed packed tensor data | Covered by source hygiene and greedy readback regression tests |
