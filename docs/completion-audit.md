...
| Realtime/Bevy defaults | Bevy defaults use target-cfg platform selection, patch-diff startup mode, adaptive display transfer, interframe output, PSNR overlay, and deduplicated mask geometry. `--mask-source autogaze` enables the NVIDIA model path with 16-frame realtime streaming cache ordering. | High-motion/full-frame camera moves still need app-level bottleneck isolation and stable FPS evidence. Live camera FPS requires a real hardware adapter run. |
